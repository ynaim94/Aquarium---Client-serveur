...
public class Aquarium {
    ...
    /*Méthode addFish provisoire pour pouvoir tester l'affichage*/
   private static void addFish(String name)
	...
	for(int j =0;j<44;j++){
	    initPosition.add(2*j);
	    initPosition.add(2*j);
	    initPosition.add(2*j+1);
	    initPosition.add(2*j+1);
	}
	Fish poiss=new Fish(name,initPosition,5,dimensions);
	fishes.add(poiss);
	contentPane.setFishes(fishes);
    }
	...
	while(cmd==""){  
		...
		if(pattern[0].matcher(response).matches()){/*OK*/
		    if(pattern[3].matcher(cmd).matches())
			{  /*addFish*/
			    Pattern p = Pattern.compile(" ");
			    // séparation en sous-chaînes
			    String[] items = p.split(cmd);
			    addFish(items[1]);
			} 
